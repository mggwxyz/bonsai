...
macOS with Homebrew, git, and zsh. Check prerequisites first — this reports anything missing along with the fix:
...
Each managed workspace uses one `.bonsai.toml`, looked up at the workspace root first, then inside the default worktree (`my-app/.bonsai.toml`, then `my-app/main/.bonsai.toml`). Keep it at the root for local-only settings; move it into the repo to share ports, commands, and URL templates with teammates.
